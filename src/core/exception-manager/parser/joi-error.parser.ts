...
const parsePath = (path: any) => {
  if (path instanceof Array) {
    ...
    path = path.reduce((current: any, value: any) => {
      return current
        .toString()
        .concat('.')
        .concat(value);
  ...
  if (arrayIndexes && arrayIndexes.length > 0) {
    arrayIndexes.forEach((index: any) => {
      const indexPosition =
        index.substring(0, 1) === '.'
...
const buildJoiError = (error: any) => {
  ...
  if (!joiError) {
    return errorParser(20020, `- Error code not found for joi error ${errorType}`);
  }

  const parameters = joiError.parameters(errorDetails[0], [parameterType, parameterPath]);

  return errorParser(joiError.code, parameters);
};

const buildGenericErrors = (error: any) => {
  ...
  return errorParser(20020, `- Error code not found for joi error ${error.output.payload.message}`);
};
