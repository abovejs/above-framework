...
export function responseSucess(data: any, options: any, customMeta: any) {
...
export function responseSuccessApi(data: any, options: any, customMeta: any) {
  options = {
    ...options,
    ...data.meta,
  };
...
export function responseError(err: any) {
  if (err.response && err.response.headers['content-type'].indexOf('application/json') === -1) {
  ...
  const errorMessage = errorParser(err.errorCode, err.parameters, err.statusCode);
  ...
  const errorBody = err.userMessage && err.developerMessage && err.moreInfo ? err : errorMessage.body;
